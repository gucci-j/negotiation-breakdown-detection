...
def read_ji_negotiations(file_name: str, breakdown_flag=False) -> List[Negotiation]:
    ...
    if breakdown_flag is True:
        ...
        # For Ordinary Tasks
        negotiations = [n for n in negotiations if n.solution is not None] #2478
        negotiations = [n for n in negotiations if n.status == "completed"] #2452 / 2639 negotiations
    
    return negotiations


def main():
    negos = read_negotiations("./data/data.json")
