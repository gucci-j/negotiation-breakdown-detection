...
def convert_to_units(text: str, player_tag=None, prev_tag=None):
    ...
    # propose
    propose_result = re.search(r"\bball\b|\bhat\b|\bbook\b|\balls\b|\bhats\b|\bbooks\b| (\$\d{1,}) | (\d{1,}\$) | \d{1,} | (come down) | (highest) | (lowest) | (go hiher) | (go lower) | (i would like)", text) 
    ...
    for list_index, (start_index, tag) in enumerate(queue):
        if list_index == 0:
            ...
            # comes with any either propose or inquire
            if tag == 'inquire':
                ret_text.append(nego_units[5])
                inquire_flag = True
                break
